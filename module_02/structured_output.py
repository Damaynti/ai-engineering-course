...
class Ticket(BaseModel):
    ...
    category: Literal["регламент", "доступ", "инцидент", "документация"] = Field(
        description="Категория обращения: регламент - если нужно узнать порядок действий или о сроках; "
                "доступ - если нужно получить, изменить или восстановить доступ; "
                "инцидент - если сообщают о поломке, аварии или неисправности; "
                "документация - если нужно найти или получить документ. "
                "Выбирай категорию по основной цели обращения, а не по отдельным словам в тексте."
    )

    equipment_id: Optional[str] = Field(
        default=None,
        description="Идентификатор оборудования. Приводи название к виду из реестра "
                "(например, КМ-101, П-7, ЭЛОУ-АВТ-6). Если оборудование в обращении не названо, укажи null."
                "Не угадывай и не выводи идентификатор из общих слов вроде \"станок\" или \"в цеху\"",
    )

    priority: Literal["низкий", "средний", "высокий"] = Field(
        description="Приоритет обращения: низкий - если ситуация не мешает работе и может быть решена позже;"
                "средний - если проблема мешает работе, но работа может продолжаться;"
                "высокий - если есть авария, остановка оборудования или серьёзная проблема, требующая быстрого решения."
    )
    ...
    def check_equipment_id(cls, value: Optional[str]) -> Optional[str]:
        ...
        raise ValueError(f"Оборудование «{value}» не найдено в реестре оборудования")          
