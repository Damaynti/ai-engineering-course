...
Обращение: "Где найти актуальную инструкцию по охране труда?" -> окументация
Обращение: "Кто отвечает за согласование доступа к 1С?" -> регламент
Обращение: "Пропал доступ к папке с чертежами после смены пароля" -> инцидент
"""
...
узнать порядок действий, получить доступ, сообщить о поломке или найти документ.
Не путай тему обращения с его целью:
если в тексте упомянута авария, но человек спрашивает про порядок оформления - это регламент, а не инцидент.
